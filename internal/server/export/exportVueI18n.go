...
func (s *Server) ExportVueI18N(
	ctx context.Context,
	req *connect.Request[pb.ExportVueI18NRequest],
) (*connect.Response[pb.ExportVueI18NResponse], error) {
	...
	// Build nested JSON structure for Vue i18n
	translations := make(map[string]interface{})

	for _, entry := range entries {
		for _, loc := range entry.Localizations {
			if loc.LocaleID == locale.ID {
				// Support nested keys with dot notation (e.g., "user.name" -> {"user": {"name": "..."}})
				setNestedKey(translations, entry.Key, loc.Translation)
				break
			}
		}
	}
	...
	return connect.NewResponse(&pb.ExportVueI18NResponse{
		Format:   "vue-i18n",
